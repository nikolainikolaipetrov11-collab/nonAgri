...
def build_interpretable_features(real_physics_matrix, quality_matrix=None):
    ...
    if quality_matrix is not None:
        df_feats['Mean_Quality_平均影像质量'] = np.mean(quality_matrix, axis=1)
        df_feats['Low_Quality_Months_低质量月份数'] = (quality_matrix < 0.5).sum(axis=1)
...
def run_trustworthy_active_learning():
    ...
    X_train = build_interpretable_features(
        full_dataset.get_raw_physical_features(train_indices),
        full_dataset.get_quality_weights(train_indices),
    )
    ...
    X_infer = build_interpretable_features(
        full_dataset.get_raw_physical_features(infer_indices),
        full_dataset.get_quality_weights(infer_indices),
    )
